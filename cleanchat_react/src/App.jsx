...
export default function App() {
    ...
    function clearUnreadForKey(key) {
        if (key.startsWith('user:')) {
            const id = key.slice(5);
            setUnreadUsers(prev => { const c = { ...prev }; delete c[id]; return c; });
        } else if (key.startsWith('group:')) {
            const id = key.slice(6);
            setUnreadGroups(prev => { const c = { ...prev }; delete c[id]; return c; });
        }
    }
    ...
    async function fetchUsers(token = localStorage.getItem('token')) {
        if (!token) return;
        try { const res = await fetch('/api/users', { headers: { Authorization: `Bearer ${token}` } }); if (res.ok) setUsers(await res.json()); } catch { }
    }

    async function fetchGroups(token = localStorage.getItem('token')) {
        if (!token) return;
        try { const res = await fetch('/api/groups', { headers: { Authorization: `Bearer ${token}` } }); if (res.ok) setGroups(await res.json()); } catch { }
    }

    async function fetchPublicHistory() {
        try { const res = await fetch('/api/messages'); if (!res.ok) return; const items = await res.json(); setMessages(prev => ({ ...prev, public: items.map(i => formatMsg(i.timestamp, i.user, i.message)) })); } catch { }
    }
    ...
    function sendTypingSignal() {
        if (!connectionRef.current) return;
        if (channel.type === 'private' && channel.id) connectionRef.current.invoke('TypingPrivate', channel.id).catch(() => { });
        else if (channel.type === 'group' && channel.id) connectionRef.current.invoke('TypingGroup', channel.id).catch(() => { });
    }

    async function connectWithToken(token) {
        if (!token) return;
        try {
            if (connectionRef.current) { try { await connectionRef.current.stop(); } catch { } connectionRef.current = null; setConnected(false); }
            ...
            connection.on('ReceiveMessage', (userName, message, timestamp) => pushMessage('public', formatMsg(timestamp, userName, message)));
            ...
            connection.on('PresenceList', setOnlineUserIds);
            connection.on('PresenceUpdated', (id, online) => setOnlineUserIds(prev => { const s = new Set(prev); online ? s.add(id) : s.delete(id); return Array.from(s); }));
            ...
            connection.on('GroupCreated', (group) => {
                if (!group) return;
                const newGroup = group.id ? group : (Array.isArray(group) ? { id: group[0], name: group[1] } : null);
                if (!newGroup) return;
            ...
            setConnected(true);
            setNotice({ severity: 'success', message: 'Connected to SignalR' });
        } catch (err) { setNotice({ severity: 'error', message: `Connect error: ${String(err)}` }); }
    }
    ...
    useEffect(() => {
        ...
        if (token) {
            const payload = parseJwt(token);
            setCurrentUser({ id: payload.sub ?? payload.nameid ?? '', name: payload.FullName ?? payload.name ?? payload.fullName ?? '' });
            fetchUsers(token); fetchGroups(token); fetchPublicHistory();
            connectWithToken(token);
        }
    }, []);
    ...
    async function loginAndConnect() {
        try {
            ...
            if (!res.ok) return setNotice({ severity: 'error', message: `Login failed: ${res.status}` });
            const json = await res.json();
            const token = json.token ?? json.accessToken;
            if (!token) return setNotice({ severity: 'error', message: 'No token returned' });
            localStorage.setItem('token', token);
            const payload = parseJwt(token);
            setCurrentUser({ id: payload.sub ?? payload.nameid ?? '', name: payload.FullName ?? payload.name ?? payload.fullName ?? '' });
            await fetchUsers(token); await fetchGroups(token); await fetchPublicHistory(); await connectWithToken(token);
        } catch (err) { setNotice({ severity: 'error', message: `Login error: ${String(err)}` }); }
    }
    ...
    async function logout() {
        ...
        connectionRef.current = null;
        setConnected(false); setUsers([]); setGroups([]); setMessages({ public: [] }); setCurrentUser({ id: '', name: '' }); setNotice({ severity: 'info', message: 'Logged out' });
    }

    async function sendMessage() {
        if (!connectionRef.current || !messageText.trim()) return;
        try {
            if (channel.type === 'public') await connectionRef.current.invoke('SendMessage', messageText.trim());
            else if (channel.type === 'private') await connectionRef.current.invoke('SendPrivateMessage', channel.id, messageText.trim());
            else if (channel.type === 'group') await connectionRef.current.invoke('SendGroupMessage', channel.id, messageText.trim());
            setMessageText('');
        } catch (err) { setNotice({ severity: 'error', message: `Send error: ${String(err)}` }); }
    }

    async function createGroup() {
        if (!newGroupName.trim()) return setNotice({ severity: 'warning', message: 'Group name required' });
        try {
            const token = localStorage.getItem('token');
            const res = await fetch('/api/groups', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + token }, body: JSON.stringify(newGroupName) });
            if (res.ok) { fetchGroups(token); setNewGroupName(''); setNotice({ severity: 'success', message: 'Group created' }); }
            else setNotice({ severity: 'error', message: `Create group failed: ${res.status}` });
        } catch (err) { setNotice({ severity: 'error', message: `Create group error: ${String(err)}` }); }
    }
    ...
    useEffect(() => {
        if (!connectionRef.current || !messageText.trim()) return;
        if (typingTimerRef.current) clearTimeout(typingTimerRef.current);
        typingTimerRef.current = setTimeout(() => { sendTypingSignal(); typingTimerRef.current = null; }, 400);
    }, [messageText]);
            ...
            <Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
                <AppBar position="static" color="primary">
                    <Toolbar>
                        <GroupIcon sx={{ mr: 1 }} />
                        <Typography variant="h6" sx={{ flexGrow: 1 }}>CleanChat</Typography>
                        {currentUser.name && <Typography variant="body2" sx={{ opacity: 0.9, mr: 2 }}>Signed in as {currentUser.name}</Typography>}
                        <TextField size="small" placeholder="email" value={email} onChange={e => setEmail(e.target.value)} sx={{ mr: 1, bgcolor: 'background.paper', borderRadius: 1 }} />
                        <TextField size="small" placeholder="password" type="password" value={password} onChange={e => setPassword(e.target.value)} sx={{ mr: 1, bgcolor: 'background.paper', borderRadius: 1 }} />
                        <Button color="inherit" onClick={connected ? logout : loginAndConnect}>{connected ? 'Logout' : 'Login & Connect'}</Button>
                    </Toolbar>
                </AppBar>
                            ...
                            <Paper sx={{ p: 2, height: '100%', display: 'flex', flexDirection: 'column' }}>
                                            ...
                                            return (
                                                <MenuItem key={u.id} value={u.id}>
                                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
                                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                            <Avatar sx={{ width: 28, height: 28, bgcolor: 'primary.light' }}>{(u.fullName ?? u.email)[0]?.toUpperCase()}</Avatar>
                                                            <span>{u.fullName ?? u.email}</span>
                                                            {online && <Box sx={{ width: 8, height: 8, bgcolor: 'green', borderRadius: '50%' }} />}
                                                        </Box>
                                                        {unread > 0 && <Badge color="secondary" badgeContent={unread} />}
                                                    </Box>
                                                </MenuItem>
                                ...
                                <FormControl fullWidth size="small" sx={{ mb: 1 }}>
                                    ...
                                    <Select labelId="group-select-label" value={channel.type === 'group' ? channel.id : ''} label="Group" onChange={e => setChannel({ type: e.target.value ? 'group' : 'public', id: e.target.value })}>
                                        <MenuItem value=""><em>Choose group</em></MenuItem>
                                        {groups.map(g => { const unread = unreadGroups[g.id] || 0; return (<MenuItem key={g.id} value={g.id}><Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}><Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><GroupIcon sx={{ color: 'primary.dark' }} /><span>{g.name}</span></Box>{unread > 0 && <Badge color="secondary" badgeContent={unread} />}</Box></MenuItem>); })}
                                    </Select>
                                </FormControl>
                        ...
                        <Grid item xs={9}>
                            <Paper sx={{ p: 2, height: '100%', display: 'flex', flexDirection: 'column' }}>
                                ...
                                <List sx={{ flexGrow: 1, overflowY: 'auto', mb: 1 }}>
                                    {currentMessages.map((m, i) => <ListItem key={i}><ListItemText primary={m} /></ListItem>)}
                                </List>
                                <Box sx={{ display: 'flex', gap: 1 }}>
                                    <TextField fullWidth size="small" value={messageText} onChange={e => setMessageText(e.target.value)} placeholder="Type a message..." />
                ...
                {notice && <Snackbar open autoHideDuration={4000} onClose={() => setNotice(null)} anchorOrigin={{ vertical: 'top', horizontal: 'center' }}>
                    <Alert onClose={() => setNotice(null)} severity={notice.severity} sx={{ width: '100%' }}>{notice.message}</Alert>
                </Snackbar>}
            </Box>
        </ThemeProvider>
    );
